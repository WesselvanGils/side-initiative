...
import { handleCommanderSocketRequest, registerSideInitiativeSocket, SideInitiativeAPI } from "../src/api.js";
...
function createCombatant({
    ...
    return {
        ...
        testUserPermission(user: UserLike | null | undefined, permission: string) {
            return (
                permission === "OWNER" && (user?.isGM || (typeof user?.id === "string" && ownerIds.includes(user.id)))
            );
        },
...
function createCombat(combatants: TestCombatant[], state: TestState = null): TestCombat {
...
test("handleCommanderSocketRequest rejects advance requests from users outside the active side", async () => {
    ...
    try {
        ...
        assert.equal(result, null);
        assert.equal(SideInitiativeAPI.getSideState(combat)?.find((side) => side.active)?.id, "players");
...
test("player side actions dispatch over the native game.socket and the GM handler applies them", async () => {
    ...
    try {
        ...
        assert.ok(registeredListener, "registerSideInitiativeSocket should register a socket listener");
        ...
        await deliverAsGm(emitCalls[0].args[0], emitCalls[0].args[1] as string | null);
        assert.equal(SideInitiativeAPI.getSideState(combat)?.find((side) => side.active)?.id, "monsters");
        assert.equal(combat.turn, 1);

        const commanderRequested = await SideInitiativeAPI.requestSideCommander(combat, player);
        ...
        await deliverAsGm(emitCalls[1].args[0], emitCalls[1].args[1] as string | null);
        assert.equal(SideInitiativeAPI.getSideCommander(combat, "players")?.id, player.id);
    } finally {
        testGlobal.game = original.game;
    }
});
