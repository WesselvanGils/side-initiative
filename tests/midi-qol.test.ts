...
test("MidiQOL continues clearing side reactions when one actor has a stale reaction effect", async () => {
    ...
    try {
        ...
        assert.deepEqual(deletes, [{ actorUuid: "actor-2-uuid", effectId: "dnd5ereaction000" }]);
        assert.deepEqual(
            updates.map((entry) => entry.actorUuid),
            ["actor-1-uuid", "actor-2-uuid"],
        );
    } finally {
        env.restore();
    }
...
test("MidiQOL skips the side commander because the active combatant reset is handled by MidiQOL", async () => {
    ...
    try {
        ...
        assert.deepEqual(deletes, [{ actorUuid: "member-uuid", effectId: "dnd5ereaction000" }]);
        assert.deepEqual(
            updates.map((entry) => entry.actorUuid),
            ["member-uuid"],
        );
...
test("MidiQOL clears a single used reaction effect when the API is unavailable", async () => {
    ...
    const combat = {
        started: true,
        combatants: [createCombatant({ id: "pc-1", sideId: "players", actor })],
        getFlag(scope, key) {
            if (scope === "side-initiative" && key === "state") {
                return {
                    activeSideId: "monsters",
