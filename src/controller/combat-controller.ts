...
type CombatMethod = (this: CombatLike, ...args: unknown[]) => Promise<unknown> | unknown;
...
export function installCombatPatches(): void {
    ...
    CombatClass.prototype.nextRound = async function (this: CombatLike, ...args: unknown[]) {
        if (isSideCombat(this)) {
            ...
            // Advance world time with the round (see getRoundTimeDelta): Foundry's
            // native nextRound passes worldTime so seconds-based effect durations
            // (Detect Magic, timed buffs) tick. setActiveSide above used roundDelta 0.
            const worldTimeDelta = getRoundTimeDelta(1);
            await this.update?.(
                { round: Math.max(1, (this.round ?? 1) + 1) },
                worldTimeDelta ? { worldTime: { delta: worldTimeDelta } } : undefined,
            );
            return this;
        }
