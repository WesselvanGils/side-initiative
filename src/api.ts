...
import { getConst, getFoundry, getSideInitiative, getSetting, hooks, isActiveGMClient } from "./runtime.js";
import type {
    CombatLike,
    CombatState,
    CombatantLike,
...
import { COMMANDER_CONTROL_OPTIONS, INITIATIVE_METHOD_OPTIONS, MODULE_ID, SETTINGS } from "./constants.js";
...
export interface SideInitiativeApi {
    ...
    requestAdvanceSide(combat: CombatLike | null | undefined, direction?: number): Promise<boolean>;
...
async function syncCombatToSide(
    ...
    if (Object.keys(updates).length) {
        // Foundry's native nextTurn/nextRound advance game time by passing
        // `worldTime: {delta}` to combat.update; seconds-based effect durations
        // (Detect Magic, minute/hour buffs) compute remaining from game.time, so
        // without this their timers never tick during side combat. A round is
        // CONFIG.time.roundTime seconds; side turns don't map to individual Foundry
        // turns, so only advance time when a round actually elapses.
        const worldTimeDelta = getRoundTimeDelta(roundDelta);
        await combat.update?.(updates, worldTimeDelta ? { worldTime: { delta: worldTimeDelta } } : undefined);
    }
...
async function rollVisibleSideDie(
    ...
    const speaker = getFoundry()?.documents?.ChatMessage?.implementation?.getSpeaker?.({
        alias: side.name,
    }) ?? { alias: side.name };
    const flavor = game?.i18n?.format?.("COMBAT.RollsInitiative", { name: side.name }) ?? `${side.name} initiative`;
...
async function rollWeightedSideInitiative(
    resolvedCombat: CombatLike,
    { random = Math.random, refresh = true }: { random?: RandomFn; refresh?: boolean } = {},
): Promise<CombatState | null> {
    if (refresh) {
        await SideInitiativeAPI.refreshCombatantSides(resolvedCombat);
    }
    ...
    for (const combatant of combatants) {
        ...
        initiativeByCombatantId[combatant.id] = Number.isFinite(combatant.initiative)
...
/**
 * Seconds of world time to advance when a side turn spans `roundDelta` rounds.
 * Mirrors Foundry's native `nextRound`, which passes `worldTime: {delta}` to
 * `Combat#update`; without it `game.time.worldTime` is frozen and seconds-based
 * effect durations (Detect Magic, minute/hour buffs) never tick during combat.
 * Read via `globalThis` so it no-ops in environments without Foundry. A round is
 * `CONFIG.time.roundTime` seconds; side turns don't map to individual Foundry
 * turns, so only whole rounds advance time. Returns 0 when round time is unset.
 */
export function getRoundTimeDelta(roundDelta: number): number {
    if (!Number.isFinite(roundDelta) || roundDelta === 0) return 0;
    const roundTime = (globalThis as { CONFIG?: { time?: { roundTime?: number } } }).CONFIG?.time?.roundTime;
    return Number.isFinite(roundTime) && roundTime ? roundDelta * roundTime : 0;
}
...
function getSocketCombat(combatId: string | null | undefined): CombatLike | null {
    return (
...
function getRequestingUser(message: SideSocketMessage, senderUserId: string | null): UserLike | null {
...
export async function handleCommanderSocketRequest(
    ...
    if (message.action === "setCommander") {
        ...
        if (!canUserControlSide(combat, getCombatantSideId(combatant), requestingUser)) return null;

        return SideInitiativeAPI.setSideCommander(combat, combatant);
    }

    if (message.action === "advanceSide") {
        if (!SideInitiativeAPI.canUserAdvanceSide(combat, requestingUser)) return null;
...
export const SideInitiativeAPI: SideInitiativeApi = {
    ...
    async requestSideCommander(combat, combatant) {
        ...
        if (!dispatched) {
            ui?.notifications?.warn?.(game?.i18n?.localize?.("SIDE-INITIATIVE.Notifications.NoGMForCommander") ?? "");
            return false;
        }
        return true;
    },
    ...
    async requestAdvanceSide(combat, direction = 1) {
        const resolvedCombat = getCombatFromArgument(combat);
        if (!resolvedCombat || !this.canUserAdvanceSide(resolvedCombat, game?.user as UserLike | undefined))
            return false;
        ...
        if (!dispatched) {
            ui?.notifications?.warn?.(game?.i18n?.localize?.("SIDE-INITIATIVE.Notifications.NoGMForAdvance") ?? "");
            return false;
        }
        return true;
    },
    ...
    canUserSetCommander(combatant, user = game?.user as UserLike | undefined, combat) {
        const resolvedCombat = getCombatFromArgument(combat);
